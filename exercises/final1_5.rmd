...
::: {.callout-tip collapse="true" title="Lösung"}
(c) $f^{\prime}(x) = -\frac{1}{x^{2}} \Rightarrow$ (Notiz: $\frac{1}{x} = x^{-1}$)
:::


### Aufgabe 4 – (Ableitung, Integral)
...
cat("(c) Bestimmen Sie die Fläche unterhalb der Funktion $f(x) = 2 \\cdot x + 2$ für $-2 \\leq x \\leq 4$.", longmcq(opts))
```
:::
...
(c) Die Stammfunktion ist $F(x)=x^2+2x$.
	...
	1. $\int_{-2}^{-1} (2x + 2)dx = x^{2} + 2x \mid_{x = -2}^{x = -1}$ <br> <br>
...
::: {.callout-tip collapse="true" title="Lösung"}
(b) Nein, weil $C \neq C^{T}$ 
 $\Rightarrow$ da $C^{T} \begin{pmatrix} 2 & 4 \\ 3 & 1 \end{pmatrix}$
:::
...
### Aufgabe 8 – (Deskription, Einführung)
...
::: {.webex-check .webex-box}
```{r, results='asis', echo = FALSE}
cat("(d) Wie lautet der Korrelationskoeffizient? ", fitb("0.86", num = TRUE), "<br>")
